...
def handoff_marker_path(cfg: PodConfig, name: str) -> Path:
    ...
    before acting and checks the marker again immediately before task deletion.
    ...
    **The unload result must be authoritative.** If the gateway is STILL alive,
    ...
    if handoff_in_progress(cfg, name):
        return subprocess.CompletedProcess(
            ...
            stdout=ended.stdout or "",
            stderr=(
                f"pod {name!r} entered a restart handoff during teardown, so its "
                "task and HOME were preserved. "
                f"Retry in a moment: kirocrew pod down {name}"
            ),
        )
...
def supervise_gateway(
    ...
    **The resource ceiling is applied here, and this is the only place it can
    ...
    try:
        ...
        # settles a missing pid across this one-statement gap and checks the marker
        # again immediately before task deletion. Liveness and handoff remain
        # separate because one atomic record changes the all-backend on-disk
        # contract.
