...
class AcpClient:
    ...
    def _restore_aside_without_clobber(aside: Path, path: Path) -> bool:
        ...
        ``os.replace`` semantics would silently overwrite whatever raced into
        the pathname after the move-aside -- most plainly a settings file the
        user just recreated, whose loss is silent and irreversible. A hard link
        is the portable atomic no-clobber publish: it fails with
        ``FileExistsError`` when the name is taken, leaving both files intact.
        On success the aside name is dropped; on refusal (or a filesystem
        without hard links) the aside stays behind as a ``.crew-gc`` temp the
        next fresh seed ignores, and the occupant of the pathname is preserved.
        """
        try:
            os.link(aside, path)
        except FileExistsError:
            logger.info(
                "%s was recreated while Crew's seed was moved aside; leaving the new file "
                "in place (the moved seed remains at %s).",
                path,
                aside.name,
            )
            return False
    ...
    @staticmethod
    def _read_settings_text(path: Path, cap: int = 1 << 20) -> str | None:
        """A bounded, non-following read of a settings file, for diagnostics.

        Same defensive shape as :meth:`_settings_path_holds` -- ``O_NONBLOCK``
        so a FIFO cannot wedge the caller, ``O_NOFOLLOW`` so a link is not
        traversed, regular-file-only, size-capped -- because by the time this
        runs the path is whatever the world left there. ``None`` on any
        ...
        try:
            ...
            if authored:
                ...
                try:
                    ...
                    # exactly as the pre-existing-file branch above does. The
                    # winner records AFTER its write, so one short bounded pause
                    # covers the write-to-durable-record window before declining
                    # -- a loser racing ahead of that persist then still shares.
                    if self._share_settings_seed_if_identical(local_settings, payload):
                        return
                    time.sleep(0.05)
                    if self._share_settings_seed_if_identical(local_settings, payload):
