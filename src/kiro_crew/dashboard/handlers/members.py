...
async def api_members(request: web.Request) -> web.Response:
    ...
    for name, agent_cfg in cfg.agents.items():
        ...
        if _mask_agent_free_text(name) != name:
            ...
            # ``_masked_config_dict`` removing a credential-shaped agent key
            # (#8717): the grammar gate above admits alphanumeric key-shaped
            # strings the redactors would alter, so it is not sufficient.
            continue
        try:
        ...
        rows.append(
            {
                ...
                # the fixed sentinel instead (#8717's rule, applied to this
                # endpoint by #8781). Benign values are byte-identical.
