            ...
            if event.kind == EVENT_TEXT_CHUNK:
                        ...
                        if _slack_approval_ts is None:
                            ...
                            if not fut.done():
                                ...
                                _host_deny_reason = (
                                    ...
                                    "to Slack, so the host declined it"
                                )
                    except Exception:
                ...
                try:
                    ...
                    _host_deny_reason = (
                        ...
                        f"{int(_approval_window)}s, so the host declined it"
                    )
                ...
                if outcome != "approved":
                    ...
                    # false and worth an in-band correction. The sentence is
                    # copied (not the DENY_CAUSE_* constant) because the cascade
                    # notice embeds it verbatim as the reason the model reads.
                    slot._batch_rejected_cause = _host_deny_reason
