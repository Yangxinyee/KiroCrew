            ...
            try:
                ...
                if run.branch_name:
                    ...
                    if not await self._ensure_resumable_workspace(run, "restart"):
                        # The worktree failed validation and could not be
                        # recovered: the finally-block must NOT finalize it.
                        # finalize() removes run.worktree_path --force, and a
                        # rejected path may be an unrelated directory (or a
                        # different repository) that merely sits where the
                        # worktree used to be. Deleting what was never
                        # positively identified as ours is the exact class
