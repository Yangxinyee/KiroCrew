...
class SlackRenderer(Renderer):
    ...
    def __init__(
        ...
        *,
        ...
        self._delivered = ""
        self._stream_delta_dropped = False  # an append delta never landed on the live stream
        self._segment_text = ""  # text belonging to the CURRENT stream message (reset on rotation)
        self._segment_delivered = ""  # confirmed appends on the CURRENT stream message
    ...
    async def _ensure_stream(self) -> str | None:
        if self._stream_ts is None:
            ...
            if ts:
                ...
                self._segment_delivered = ""
                if not self._stream_delta_dropped:
                    # A fresh stream message begins: the recovery segment
                    # tracks the current message only. Unsettled debt carries
                    # forward.
                    self._segment_text = ""
            else:
    ...
    async def _rotate_stream(self) -> str | None:
        ...
        """
        if self._stream_ts:
            if self._stream_delta_dropped:
                # The message being abandoned is missing dropped deltas, and
                # once rotation moves on nothing can reach it again — make it
                # whole NOW by re-rendering its segment over it. Mid-turn, so
                # best-effort: on success the debt is settled and the flag
                # clears; on failure the flag AND the segment are retained, so
                # the debt carries into the new message and ``on_done``
                # delivers it there (never silently discarded).
                try:
                    seg = _display_safe(self._segment_text)
                    seg_chunks = await self._split_for_slack(seg or _NO_RESPONSE)
                    await self.slack.update_message(self.channel, self._stream_ts, seg_chunks[0])
                    # Ledger: the update REPLACED this message — retract the
                    # superseded appends, then record each confirmed part.
                    self._ledger_replace_current(seg_chunks[0])
                    for seg_part in seg_chunks[1:]:
                        await self.slack.post_message(self.channel, seg_part, self.thread_ts)
                        self._delivered += seg_part
                    self._stream_delta_dropped = False
                except Exception:
                    logger.warning(
    ...
    async def _append_stream(self, text: str) -> bool:
        ...
        if not ok and self._use_slack_stream:
            ...
            # re-renders from ``_accumulated`` — that path recovers on its
            # own). Flag it so ``on_done`` re-renders the segment instead of
            # recording success on partial content: the real ``stop_stream``
            # deliberately ignores ``final_text``.
            self._stream_delta_dropped = True
        if ok:
            ...
            # Per-message confirmed portion: a recovery REPLACES the current
            # message, so the ledger must be able to retract exactly what that
            # replace superseded.
            self._segment_delivered += text
        return ok

    def _ledger_replace_current(self, first_part: str) -> None:
        """Reconcile the delivery ledger for a message REPLACE.

        A recovery ``chat.update`` supersedes the current message's confirmed
        appends with ``first_part``, so those appends are retracted from the
        ledger before the replacing part is recorded — otherwise the rescue
        would persist the same content twice.
        """
        if self._segment_delivered and self._delivered.endswith(self._segment_delivered):
            self._delivered = self._delivered[: -len(self._segment_delivered)]
        self._delivered += first_part
        self._segment_delivered = first_part
    ...
    async def on_tool_call(
        ...
        if tool_name == "wait" and self._use_slack_stream and self._stream_ts:
            ...
            if self._ref_hold:
                ...
                self._ref_hold = ""
            if self._stream_delta_dropped:
                # A dropped delta is normally recovered by ``on_done``, but
                # this finalize abandons the message — recover it here, over
                # the message it belongs to. Delivery is confirmed: on success
                # the debt is settled; on failure the flag and segment are
                # retained, so the debt carries into the post-wait message
                # rather than being discarded.
                try:
                    rec = _display_safe(self._segment_text)
                    rec_chunks = await self._split_for_slack(rec or _NO_RESPONSE)
                    await self.slack.update_message(self.channel, self._stream_ts, rec_chunks[0])
                    self._ledger_replace_current(rec_chunks[0])
                    for rec_part in rec_chunks[1:]:
                        await self.slack.post_message(self.channel, rec_part, self.thread_ts)
                        self._delivered += rec_part
                    self._stream_delta_dropped = False
                except Exception:
                    logger.warning(
                        "Wait-finalize recovery failed — carrying the segment debt forward",
                        exc_info=True,
                    )
    ...
    async def on_done(self, stop_reason: str = "") -> None:
        ...
        if self._stream_ts is not None:
            if self._use_slack_stream:
                ...
                if self._stream_delta_dropped:
                    # An append delta never landed on the current stream
                    # message and ``stop_stream`` ignores ``final_text`` — the
                    # message is missing content. Re-render ONLY the current
                    # message's segment (rotation resets it, so text an
                    # earlier, abandoned message already shows is never
                    # duplicated). Delivery is confirmed: on a raise the
                    # content is genuinely missing, so the raise propagates to
                    # the transport catch-all — and ``_finalized`` is reset
                    # FIRST, because it was set at entry and would otherwise
                    # tell the dispatcher's partial-progress rescue the turn
                    # completed, suppressing the rescue of the very output
                    # this recovery failed to deliver.
                    try:
                        seg = _display_safe(self._segment_text)
                        seg_chunks = await self._split_for_slack(seg or _NO_RESPONSE)
                        await self.slack.update_message(
                            self.channel, self._stream_ts, seg_chunks[0]
                        )
                        self._ledger_replace_current(seg_chunks[0])
                        for seg_part in seg_chunks[1:]:
                            await self.slack.post_message(self.channel, seg_part, self.thread_ts)
