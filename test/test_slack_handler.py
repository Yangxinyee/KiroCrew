...
class TestStreamingLoopPresentationGuards:
    ...
    @pytest.mark.asyncio
    async def test_drop_recovery_of_long_segment_is_lossless(self):
        """A recovered segment longer than one Slack message must arrive whole
        as ordered messages (update + overflow posts), not be cut at the
        single-message render's ceiling."""
        ...
        long_body += "\nTAIL-MARKER-OMEGA."
        slack = RaisingAppendSlack()
        provider = FakeProvider([LLMEvent(kind="text_chunk", text=long_body + "\n")])
        sessions = self._TrackingSessions(provider)

        await handle_message(slack, sessions, "C1", "hello", None, "msg1", "U1")

        delivered = " ".join(
            str(a[1].get("text") or "") for a in slack.actions if a[0] in ("update", "post")
        )
        assert "row 0" in delivered, slack.actions
        # The tail past the per-message limit must arrive via overflow posts.
        assert "TAIL-MARKER-OMEGA" in delivered, slack.actions
        assert sessions.record_failure_calls == 0

    @pytest.mark.asyncio
    async def test_drop_recovery_failure_records_turn_failure(self):
        """Success accounting has already run when the end-of-turn recovery
        executes; a recovery failure must be converted into an explicit
        failure record, and the persisted history row must carry the
        partial-turn marker instead of recording unseen output as
        established fact."""
        from unittest.mock import MagicMock

        from kiro_crew.slack.renderer import PARTIAL_TURN_MARKER

        class RaisingAppendAndUpdateSlack(MockSlackClient):
            def __init__(self):
                super().__init__()
                self._stream_enabled = True
        ...
        slack = RaisingAppendAndUpdateSlack()
        provider = FakeProvider([LLMEvent(kind="text_chunk", text="The answer is 42.\n")])
        sessions = self._TrackingSessions(provider)
        log = MagicMock()
        log.get_metadata.return_value = {}
        ...
        await handle_message(
            slack, sessions, "C1", "hello", None, "msg1", "U1", conversation_log=log
        )

        assert sessions.record_failure_calls >= 1, slack.actions
        # The assistant row persisted for this turn must be marked partial.
        assistant_rows = [
            c.args[2]
            for c in log.append.call_args_list
            if len(c.args) >= 3 and c.args[1] == "assistant"
        ]
        assert assistant_rows, log.append.call_args_list
        assert any(PARTIAL_TURN_MARKER in r for r in assistant_rows), assistant_rows
