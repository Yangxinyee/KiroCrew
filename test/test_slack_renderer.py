...
class TestRendererPresentationGuards:
    ...
    def test_raising_append_stream_recovers_full_text(self):
        """Append raises, rotation succeeds, retry raises: the delta never lands
        while the stream stays the delivery surface — ``on_done`` must re-render
        the full text over the stream (the real ``stop_stream`` ignores
        ``final_text``)."""
        ...
        # Recovery must land the full text via update_message (the dropped-delta
        # re-render), not merely stop_stream final_text.
        updates = [str(kw.get("text") or "") for m, kw in rec.calls if m == "update_message"]
        assert any("The answer is" in u for u in updates), rec.calls
    ...
    def test_wait_finalize_recovers_dropped_prewait_text(self):
        """A delta dropped BEFORE a wait finalize would be cleared with
        ``_accumulated`` — the recovery must land the pre-wait text over the
        ...
        renderer = SlackRenderer(rec, "C1", "t1", reactions_enabled=False)
        events = [
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="part one before wait"),
            AcpEvent(
                kind=EVENT_TOOL_CALL, tool_call_id="w", title="Running: wait", tool_final=False
            ),
            AcpEvent(kind=EVENT_COMPLETE, stop_reason="end_turn"),
        ]
        asyncio.run(TurnDriver(_Provider(events), renderer, approval_mode="auto").run("hi"))
        updates = [str(kw.get("text") or "") for m, kw in rec.calls if m == "update_message"]
        # The driver's rolling redactor may withhold the trailing word, so
        # assert on the confirmed-delivered prefix.
        assert any("part one before" in u for u in updates), rec.calls

    def test_drop_recovery_is_segment_scoped_no_duplication(self):
        """Chunk A lands on stream 1; the stream then refuses, chunk B rotates
        to stream 2 and its retry raises. Recovery must re-render only the
        current message's segment (B), never re-showing A."""

        class FailAfterFirstAppendSlack(_RecSlack):
            def __init__(self):
                super().__init__()
        ...
        rec = FailAfterFirstAppendSlack()
        renderer = SlackRenderer(rec, "C1", "t1", reactions_enabled=False)
        events = [
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="ALPHA-SEGMENT-ONE.\n"),
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="BRAVO-SEGMENT-TWO.\n"),
            AcpEvent(kind=EVENT_COMPLETE, stop_reason="end_turn"),
        ]
        asyncio.run(TurnDriver(_Provider(events), renderer, approval_mode="auto").run("hi"))
        updates = [str(kw.get("text") or "") for m, kw in rec.calls if m == "update_message"]
        recovery = [u for u in updates if "BRAVO-SEGMENT" in u]
        assert recovery, rec.calls
        assert not any("ALPHA-SEGMENT-ONE" in u for u in recovery), rec.calls

    def test_drop_recovery_failure_propagates(self):
        """The dropped-delta recovery update is the answer-carrying call: when
        it raises too, the content is genuinely missing, so the raise must
        propagate (the transport catch-all records the failure and runs its
        partial-progress rescue) rather than record success on partial
        content."""
        import pytest

        class RaisingAppendAndUpdateSlack(_RecSlack):
            async def append_stream(self, channel, ts, text):
                raise RuntimeError("ratelimited: chat.appendStream")

            async def update_message(self, channel, ts, text="", blocks=None):
                raise RuntimeError("message_not_found: chat.update")

        rec = RaisingAppendAndUpdateSlack()
        renderer = SlackRenderer(rec, "C1", "t1", reactions_enabled=False)
        with pytest.raises(RuntimeError):
            asyncio.run(
                TurnDriver(_Provider(self._events()), renderer, approval_mode="auto").run("hi")
            )
        # The raise must leave the renderer NOT finalized: ``turn_finalized``
        # is what the dispatcher's partial-progress rescue consults, and a
        # finalized turn suppresses the rescue of the very output this
        # recovery failed to deliver.
        assert renderer.turn_finalized is False, renderer.turn_finalized

    def test_failed_rotation_recovery_carries_debt_forward(self):
        """When the rotation-time recovery update fails, the segment debt must
        survive the rotation and be delivered by ``on_done``'s recovery."""

        class DebtSlack(_RecSlack):
            def __init__(self):
                super().__init__()
            ...
            async def update_message(self, channel, ts, text="", blocks=None):
                ...
                return await super().update_message(channel, ts, text=text, blocks=blocks)

        rec = DebtSlack()
        renderer = SlackRenderer(rec, "C1", "t1", reactions_enabled=False)
        events = [
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="ALPHA-DEBT-ONE.\n"),
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="BRAVO-DEBT-TWO.\n"),
            AcpEvent(kind=EVENT_COMPLETE, stop_reason="end_turn"),
        ]
        asyncio.run(TurnDriver(_Provider(events), renderer, approval_mode="auto").run("hi"))
        updates = [str(kw.get("text") or "") for m, kw in rec.calls if m == "update_message"]
        assert any("ALPHA-DEBT-ONE" in u and "BRAVO-DEBT" in u for u in updates), rec.calls

    def test_recovery_parts_recorded_in_delivery_ledger(self):
        """Confirmed recovery deliveries must advance the ``delivered_text``
        ledger (the dispatcher's rescue persists only that), and a recovery
        REPLACE must retract the appends it superseded so the ledger never
        holds the same content twice."""

        class FailSecondAppendSlack(_RecSlack):
            def __init__(self):
                super().__init__()
                self._appended = 0
        ...
        rec = FailSecondAppendSlack()
        renderer = SlackRenderer(rec, "C1", "t1", reactions_enabled=False)
        events = [
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="ALPHA-LEDGER-ONE.\n"),
            AcpEvent(kind=EVENT_TEXT_CHUNK, text="BRAVO-LEDGER-TWO.\n"),
            AcpEvent(kind=EVENT_COMPLETE, stop_reason="end_turn"),
        ]
        asyncio.run(TurnDriver(_Provider(events), renderer, approval_mode="auto").run("hi"))
        shown = renderer.delivered_text
        # The recovered segment is in the ledger...
        assert "BRAVO-LEDGER" in shown, shown
        # ...and nothing is recorded twice.
        assert shown.count("ALPHA-LEDGER-ONE") <= 1, shown
        assert shown.count("BRAVO-LEDGER-TWO") <= 1, shown
