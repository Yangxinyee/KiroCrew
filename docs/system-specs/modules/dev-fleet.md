...
worktree exists: `{lane, name, worktree, ref, version, error, at_tip, behind,
name_taken_by_branch}`. Every field has a reader: `name` is what labels the
...
resolved commit id and the worktree's own HEAD are deliberately absent — a field
...
That `error` also reaches the page's `ErrorNotice`, not just the row. It arrives on the
FLEET POLL rather than from a button, so nothing routed it there and it showed only as
muted text inside a dimmed placeholder — no agent hand-off, nothing to dismiss. The
notification is keyed on the error TEXT rather than on the poll, because the fleet
refreshes every 12s and re-notifying each cycle would bury the page in duplicates.
