...
Loopback records are created through the API or CLI. Settings does not offer
loopback in its add form. Settings does render an existing loopback record with
the `Loopback (same host)` method and fixed `127.0.0.1` target. Its editor keeps
`connection_method="loopback"`, requires no SSH host, and exposes only the
fields the record uses: name, remote port and token TTL.
