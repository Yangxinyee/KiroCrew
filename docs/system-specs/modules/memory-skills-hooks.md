...
- **Staged approval:** new skills route to `auto/.pending/<slug>/`; approve promotes to `auto/<slug>/` (dashboard: Skills → Pending review). Auto-approve for prose-only is opt-in via `approval_required=false`; **candidates with a validated script always require approval**. Scripts rejected by the static validator are dropped, leaving the candidate prose-only, so it may auto-publish when `approval_required=false`.
