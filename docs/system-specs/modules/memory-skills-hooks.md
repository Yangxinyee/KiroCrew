...
refused in every category. A behavioral imperative such as
"always use <concrete ID>" is also refused in every category, so relabeling that
imperative as `knowledge` cannot bypass the boundary. A direct imperative can start the
lesson or follow `.`, `!`, `?`, or a newline, with optional `please` / `kindly` and
emphatic `do` prefixes. A concrete model ID used in a descriptive sentence without an
imperative remains valid in a `knowledge` lesson. The `negative`
