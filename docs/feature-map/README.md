...
| Work ledger | The shared record between a conductor and its workers: workers report against their bound item, and the conductor reads the reports and records acceptance. Mounted by `kirocrew-ledger-conductor`, `kirocrew-worker`, and member sessions on supported member harnesses; existing `kirocrew-conductor` and `kirocrew-pipeline-conductor` templates keep their transcript-based procedure | Agent-written via `kirocrew-work`; owner task capture and monitoring in Crew Members → **Tasks** | `pages/members/MemberTasks.tsx` | `handlers/work_ledger.py`, `handlers/member_work.py` | `GET /api/work-ledger`, `POST /api/work-ledger/record`, `POST /api/work-ledger/report`, `GET,POST /api/members/{slug}/work` |
