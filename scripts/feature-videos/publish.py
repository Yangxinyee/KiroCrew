...
def _snapshot_asset(input_dir: Path, staging: Path, name: str, *, max_bytes: int) -> Path:
    ...
    The cap is enforced on the COPY's byte count rather than the source's stat,
    because the copy is what would be published.
    """
    staged = staging / name
    with open_regular_file(input_dir / name, where="input") as source:
        written = source.copy_to(staged)
    if written == 0:
        raise ManifestError(f"{name} is empty")
    if written > max_bytes:
        raise ManifestError(f"{name} is {written} bytes, over the {max_bytes} byte cap")
    return staged
...
def _write_output(
    ...
    *,
    ...
    pending = Path(
        tempfile.mkdtemp(prefix=f".{out_dir.name}.", suffix=".pending", dir=out_dir.parent)
    )
    try:
        ...
        # mkdtemp is owner-only; a release folder is read for upload.
