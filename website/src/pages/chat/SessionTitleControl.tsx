...
import { fetchSlots, sseSlotTitle } from '../../store/dashboardSlice'
...
export default function SessionTitleControl({
  ...
  const commit = (draft: string) => {
    ...
    // Optimistic: the bar shows the new title at once. A rejected rename
    // re-pulls the authoritative slot list instead of undoing locally: the
    // server's title is the truth, so this also settles a slow failure that
    // lands after a newer rename (here or in another pane) already succeeded,
    // with no per-rename bookkeeping to get wrong. The host still reports it.
    dispatch(sseSlotTitle({ key: slotKey, title: next }))
    api.renameSlot(slotKey, next).catch((e) => {
      dispatch(fetchSlots())
